...
export default function ShipmentsPage() {
  ...
  return <div className="dashboard simple-page">
    ...
    {loading ? <p className="message">Loading shipments...</p> : shipments.length === 0 ? <p className="message">No shipments found.</p> : <section className="panel shipments-table"><div className="shipment-table-header"><span>Shipment</span><span>Destination</span><span>Carrier</span><span>Status</span><span>Actions</span></div>{shipments.map((shipment) => <article className="shipment-table-row" key={shipment.id}><div><strong>{shipment.id}</strong><small>Order {shipment.orderId}</small></div><div>{shipment.address?.city ?? "—"}{shipment.address?.state ? `, ${shipment.address.state}` : ""}</div><div>{shipment.carrier?.name ?? "Not assigned"}</div><div><span className={`status ${shipment.status}`}>{statusLabel(shipment.status)}</span></div><div className="shipment-actions">{shipment.status === "CarrierAssigned" && <button disabled={busyId === shipment.id} onClick={() => void runAction(shipment, "packing")}>Complete packing</button>}{shipment.status === "Dispatched" && <><button disabled={busyId === shipment.id} onClick={() => setTrackingShipment(shipment)}>Register tracking</button><button disabled={busyId === shipment.id} className="deliver" onClick={() => void runAction(shipment, "deliver")}>Deliver</button></>}</div></article>)}</section>}
    {trackingShipment && <div className="modal-backdrop" role="presentation"><form className="tracking-modal" onSubmit={submitTracking}><h2>Register tracking event</h2><p>{trackingShipment.id}</p><label>Description<input name="description" required placeholder="Shipment reached sorting center" /></label><label>Location<input name="location" required placeholder="São Paulo, SP" /></label><div><button type="button" onClick={() => setTrackingShipment(undefined)}>Cancel</button><button className="dispatch-button" disabled={busyId === trackingShipment.id} type="submit">Save event</button></div></form></div>}
  </div>;
}
