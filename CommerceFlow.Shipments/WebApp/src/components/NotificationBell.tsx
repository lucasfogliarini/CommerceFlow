...
export default function NotificationBell() {
  ...
  useEffect(() => {
    if (!notificationsEnabled || !authenticated || !keycloak?.token) return;
      const connection = new HubConnectionBuilder().withUrl(process.env.NEXT_PUBLIC_NOTIFICATION_HUB_URL!, { accessTokenFactory: () => keycloak.token ?? "" }).withAutomaticReconnect().build();
